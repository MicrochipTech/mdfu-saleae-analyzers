...
cmdCodes = {1:"cmdGetTargetBufferInfo",
            2:"cmdStartTransfer",
            3:"cmdWriteChunk", 
            4:"cmdGetImageState",
            5:"cmdEndTransfer"}

cmdResponseCodes = {0:"NOTINITIALIZED",
                    1:"CMDSUCCESS",
                    2:"CMDNOTSUPPORTED",
                    3:"CMDNOTAUTHORIZED",
                    4:"PACKETTRANSPORTFAILURE",
                    5:"ABORTFILETRANSFER"} 
...
class TargetBufferInfo:
    def __init__(self):
        self.cmd      = Command(cmdAResp.cmds.GetValue("cmdGetTargetBufferInfo"), dataByteArray=bytearray(0)) 
...
class StartTransfer:
    def __init__(self):
        self.cmd      = Command(cmdAResp.cmds.GetValue("cmdStartTransfer"), dataByteArray=bytearray(0)) 
        self.response = Response()
      
class WriteChunk:
    def __init__(self, fileChunkByteArray):
        self.cmd      = Command(cmdAResp.cmds.GetValue("cmdWriteChunk"), dataByteArray=fileChunkByteArray) 
        self.response = Response()
           
class GetImageState:
    def __init__(self):
        self.cmd      = Command(cmdAResp.cmds.GetValue("cmdGetImageState"), dataByteArray=bytearray(0)) 
        self.response = Response()
           
class EndTransfer:
    def __init__(self):
        self.cmd      = Command(cmdAResp.cmds.GetValue("cmdEndTransfer"), dataByteArray=bytearray(0)) 
        self.response = Response()
